...
@Suppress("unused")
class JsonSchemaMinContainsValidationTest : FunSpec() {
  init {
    JsonSchema.fromDefinition(
      ...
      test("validation passes when contains more items") {
      ...
      test("validation fails when contains less items") {
        ...
        errors.shouldContainExactly(
          ValidationError(
            schemaPath = JsonPointer("/minContains"),
            objectPath = JsonPointer.ROOT,
            message = "array must contain at least 2 element(s) match the 'contains' schema but has 1",
          ),
        )
      }
